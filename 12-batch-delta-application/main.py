...
def build_pair_counter(type_seqs: list[array], type_freqs: list[int]) -> Counter[int]:
...
def _apply_batched_deltas(
    ...
    for pair_key, delta in deltas.items():
        if delta == 0:
            continue
        changed_pair_keys.add(pair_key)

...
def _encoded_types_iter(s: str) -> Iterable[str]:
    n = len(s)
    i = 0
    first_word = True
    ...
    while i < n:
        ...
        word = s[start:i]
        if first_word:
            yield tuple(word.encode("utf-8"))  # first word: no leading space
            first_word = False
        else:
            yield tuple(("Ġ" + word).encode("utf-8"))  # subsequent words: add leading space
...
def build_pair_counter(types_counter: Counter[tuple[int, ...]]) -> Counter[tuple[int, int]]:
    ...
    logger.debug("Counter pairs, total unique pairs: %d", len(decimal_bytes_pair_counter))
